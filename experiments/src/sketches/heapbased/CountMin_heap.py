import numpy as np 
from experiments.src.sketches.basic.CountMin import CountMin
from experiments.src.sketches.heapbased.heap import Heap
import bottleneck as bn

class CountMinHeap():
    """
    CountMin with a heap to store the most frequent elements.
    ...
    def __init__(self, n: int, S_total: int, seed:int, non_neg: bool = True, L: int = 3, S_heap: int = 0):
        ...
        self.sketch = CountMin(n, int((S_total - S_heap)/L), L, preds= {}, seed=seed)
    ...
    def estimate(self, x: int) -> float:
        ...
        if x in self.heap:
            return self.heap[x]
        estimate = self.sketch.estimate(np.int64(x))
        return estimate
